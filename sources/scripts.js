...
var stage = 0;
var fullSelectorHTML = "";
var fullGameAreaHTML = "";
var gameAreaLeft  = "";
var gameAreaCurrentPlayer = "";
var gameArearight = "";
var gameAreaFooter = "";
...
{
    //$("#stage").text("NextLeftScene: " + stage.optionLeftNext + " || " + "Stage: " + stage.optionLeftNext +  " || " + "NextRightScene: " + stage.optionRightNext);
   $("#leftChoice").remove();
   $("#rightChoice").remove();
   $("#footer").remove();
   $("#gameArea").append(gameAreaLeft);
   $("#currentPlayer").after(gameAreaFooter);
   $("#currentPlayer").after(gameArearight);

   $("#leftChoice p").text(stage.optionLeft);
   $("#leftChoice p").text(stage.getSceneText);
   $("#leftChoice p").text(stage.optionright);


   $("#leftChoice").click(function ()
   {
       var currentStage = localStorage.setItem(stage.optionLeftNext)
       var newStage = getStage(stage.optionLeftNext);
       setStage(newStage); 
   });
   $("#rightChoice").click(function ()
   {
       var nextStage = stage.optionRightNext();
        localStorage.setItem(stage.optionRightNext)
        currentStage
       var newStage = getStage(stage.optionLeftNext);
       setStage(newStage); 
   });
...
function characterSceneCleanup(){
    $("#stage").after(fullGameAreaHTML);
    var currentStage = getStage(0);
...
function domPrep(){
    fullSelectorHTML = $(".playerCards").clone();
    fullGameAreaHTML = $("#gameArea").clone();
    gameAreaLeft = $("#leftChoice").clone();
    gameAreaCurrentPlayer = $("#currentPlayer").clone();
    gameArearight = $("#rightChoice").clone();
    gameAreaFooter = $("#footer").clone();
    $(".playerCards").remove();
    $("#gameArea").remove();
}
...
function loadEvents()
{
    ...
    $("#leftChoice").click(function ()
    {
        var currentStage = localStorage.getItem("stage")*1;
        var newLeft = (getStage(currentStage)).optionLeftNext;
        localStorage.setItem("stage", newLeft);
        var stage = getStage(newLeft);
        changeText(stage.optionLeft,stage.optionRight,stage.sceneText);
        $("#stage").text("NextLeftScene: " + stage.optionLeftNext + " || " + "Stage: " + newLeft +  " || " + "NextRightScene: " + stage.optionRightNext);
    });
    $("#rightChoice").click(function () {
        var currentStage = localStorage.getItem("stage")*1;
        var newRight = (getStage(currentStage)).optionRightNext;
        localStorage.setItem("stage", newRight);
        var stage = getStage(newRight);
        changeText(stage.optionLeft,stage.optionRight,stage.sceneText);
        $("#stage").text("NextLeftScene: " + stage.optionLeftNext + " || " + "Stage: " + newRight +  " || " + "NextRightScene: " + stage.optionRightNext);
    });
